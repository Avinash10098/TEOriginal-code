...
import getAllotteesInfo from '@salesforce/apex/MyBookingController.getAllotteesInfo'
...
export default class TEPortalBookings extends NavigationMixin(LightningElement) {
 ...
 @track ApplicantList = [];
@track applicantError;
@track isApplicantLoading = false;
  
  // changes on the 20-feb-2026
  @track allotteesList;
...
// changes made on the Date 20 - feb -2026 to display the Applicant Detail
@wire(getAllotteesInfo,{BookIdForPayment:'$selectedOption'})
getAllotteesInfo({ error, data }) {
  
  if (data) {

    // Add serial number dynamically
    this.allotteesList = data.map((item, index) => {
      return {
        ...item,
...
// changes made on the Date 20 - feb -2026 to display the Applicant Detail End
...
window.console.log('error <===> '+event.currentTarget.dataset.id);
    ...
    type: 'standard__webPage',
    attributes: {
      url: 'https://sbdev-tecommunity.cs31.force.com/sfc/servlet.shepherd/document/download/'+event.currentTarget.dataset.id
  }
}, false 
);
}
